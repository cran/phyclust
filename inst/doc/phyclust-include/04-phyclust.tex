...
These choices are listed in \code{.substitution}.
...
The initialization method (\code{.init.method})
...
used to model evolution in phylogenetic clustering (in \code{.substitution}).
...
to generate a template (such as \code{.EMC}) which describes
...
and use \code{X.class} to indicate the classification which can be
{\color{red}
a result (\code{class.id}) of the \code{phyclust()} function
or specified by simulations (\code{ms+seqgen}).}
In simulations, \pkg{phyclust} uses
the format ``\code{class.id-sequence.id}'' to store the sequence name.
We can apply the \proglang{R} function \code{gsub()}
(for details, type \code{?gsub}) to obtain
the classification of sequences (\code{class id}).
In this section, we demonstrate a simulated dataset,
and it can be applied to real dataset analyzed by the \code{phyclust()}
function.

For example, the following code produces the plot of Figure~\ref{fig:toydots}.
...
which may not be the first sequence/row of \code{X} if the data were not
sorted by the classification id.
If the \code{X.class} is omitted (i.e. \code{plotdots(X)}), then the first
...
The dashed lines split the clusters; in general we will not know the cluster membership before running \code{phyclust()}.
...
Next, we may like to visualize clusters on a more traditional diagram of evolutionary relationships, the phylogenetic tree.
...
\code{N.X.org} for the number of sequences,
\code{N.X.unique} for the number of unique/distinct sequences,
\code{L} for the number of sites, and
\code{N.seq.site} for the number of segregating sites.
The reports for the model are
\code{K} for the number of clusters,
\code{p} for the number of parameters,
\code{logL} for the likelihood value,
\code{bic}, \code{aic} and \code{icl} for BIC, AIC and ICL,
\code{identifier} for $\vect{Q}_k$ and $t_k$,
\code{Eta} for $\vect{\eta}$, and
\code{Tt} for $t_k$.
The default is that \code{EE} for \code{identifier} and
\code{JC69} \citep{Jukes1969} for $\vect{Q}_k$.

The \code{phyclust()} function returns a list object of class
{\color{red} \code{phyclust}}.
You can type \code{str(ret.1)} to have the details of the \code{phyclust}
class, or type \code{?phyclust} to see the descriptions of all
elements.
...
With no arguments, it retusn the default values. The internal object \code{.EMC}
...
There is a convenient function \code{find.best()} that is useful for finding
the highest likelihood value fit among multiple calls to \code{phyclust()} with varying arguments.
This function attempts to run \code{phyclust()} repeatedly on
...
by taking in a fitted model, the output of a previous call to \code{phyclust()}.
It utilizes the functions \code{ms()} and \code{seqgen()}
...
The \code{ret.4} is the result from \code{phyclust()},
\code{seq.data.toy.new} is a new dataset bootstrapped from
the model in \code{ret.4}, and we refit the same model to
this new data set and store in \code{ret.4.new} as one
bootstrap result.
