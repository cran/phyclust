...
``\code{class.id-sequence.id},'' so the \proglang{R}
...
which is only the first sequence of \code{X} if the data were sorted by the classification id.
If the \code{X.class} is omitted (i.e. \code{plotdots(X)}), then it shows changes with respect to the first
